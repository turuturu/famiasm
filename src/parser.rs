...
impl Parser {
    pub fn new() -> Parser{
        Parser {
            ...
            current_address: RamAddress{bank:0,address:0},
            insts: vec![],
        }
    }
    fn get_operand(tokens: &Vec<Annot<TokenKind>>, offset: usize, bank: u8) -> Option<Operand> {
        ...
        for tokens in token_lines {
            ...
            match &tokens[0].value {
                TokenKind::LabelDef(label) => {
                    ...
                    self.symtab.insert(label.iter().collect::<String>(), address);
                    continue;
                },
                TokenKind::Opcode(x) => {
                    ...
                    match &op {
                        ...
                        | Opcode::BVC
                        | Opcode::BVS => {
                            let operand = get_operand(&tokens, 1, current_bank);
                    ...
                    match next {
                        ...
                        TokenKind::LParen => {
                            ...
                            let operand = get_operand(&tokens, 2, current_bank);
                            let next = &tokens[2].value;
                            let addressing = match next {
                                TokenKind::Adr8(_) => {
...
pub fn parse(token_lines: Vec<Vec<Token>>) {
    println!("parse");
    let mut current_bank = 0;
    let mut current_address = 0;
    ...
    for tokens in token_lines {
        ...
        match &tokens[0].value {
            TokenKind::LabelDef(label) => {
                println!("labelDef({:?})", label);
                symtab.insert(label.iter().collect::<String>(), address);
                continue;
            },
            TokenKind::Opcode(x) => {
                println!("{:?}, Opcode(x) => {:?}", token_length, x);
